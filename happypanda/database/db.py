...
def add_db_revisions(old_db):
    ...
    log_d('Updating DB happypanda')
    c.execute('UPDATE happypanda SET happypanda=? WHERE 1', (db_constants.CURRENT_DB_VERSION,))
    conn.commit()
    conn.close()
    return
...
def check_db_version(conn):
    "Checks if DB happypanda is allowed. Raises dialog if not"
    vs = "SELECT happypanda FROM happypanda"
    c = conn.cursor()
    c.execute(vs)
    ...
    if db_vs[0] not in db_constants.DB_VERSION:
        ...
        log_c(msg)
        log_d('Local database happypanda: {}\nProgram database happypanda:{}'.format(db_vs[0],
